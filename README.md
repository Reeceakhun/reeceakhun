<p align="center">
  <img src="https://raw.githubusercontent.com/Reeceakhun/Reeceakhun/main/assets/banner" width="100%">
</p>

<h2 align="center">Hello <img src="https://user-images.githubusercontent.com/74038190/214644145-264f4759-7633-441e-9d67-d8dda9d50d26.gif" width="30px"></h2>

<!-- Animation Typing -->
<p align="center">
  <a href="https://github.com/DenverCoder1/readme-typing-svg"><img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1100&width=500&lines=I'm+Reece+Asoloko+Sabo;I'm+a+DevOps+Engineer/Technical+Specialist;Ask+me+about+OIDC+and+Workload+Identity"></a>
</p>
<!-- Animation Typing: END -->

<p align="center">
  <a href="https://www.linkedin.com/in/reece-sabo/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://www.credly.com/users/reece-asoloko-sabo/badges">
    <img src="https://img.shields.io/badge/Credly-FF6B00?style=for-the-badge&logo=credly&logoColor=white" />
  </a>
  <a href="mailto:reeceakhun@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

## 🔍 How I Work

I debug in public. My commit history isn't a highlight reel — it includes the OIDC subject mismatches, the wrong PAT permission scope, the Git Bash path-mangling that took three tries to spot. I document these in READMEs rather than squashing them away, because the fix is only half the story; the reasoning behind it is what actually transfers to the next problem.

I'd rather understand *why* something broke than just get it passing. When a role assignment turned out broader than it needed to be, or a service account had both a static key and a Workload Identity binding that made the key redundant, I'd rather flag it and explain the tradeoff than leave it unexamined.

## 🚀 Featured Projects

| Project | What it shows |
|---|---|
| [ci-cd-flask-k8s-pipeline](https://github.com/Reeceakhun/ci-cd-flask-k8s-pipeline) | 3-stage GitHub Actions pipeline: test → build & push a Docker image → deploy to an ephemeral Kubernetes cluster. Includes a Jenkins-equivalent pipeline. |
| [aks-ephemeral-infra](https://github.com/Reeceakhun/aks-ephemeral-infra) | Separate infra repo that provisions short-lived AKS clusters on request (via cross-repo trigger) and tears them down automatically via a scheduled reaper — OIDC-authenticated, no stored credentials. |
| [reece-project](https://github.com/Reeceakhun/reece-project) | Terraform provisioning a private GKE cluster on GCP (VPC, node pool, scoped service account, Artifact Registry), with nginx deployed via Helm. |

## 🔧 Languages & Tools

<p align="left">
  <img src="https://skillicons.dev/icons?i=gcp,azure,terraform,docker,kubernetes,jenkins,git,github,linux,bash" />
</p>

**Other:** Kubernetes OIDC/Workload Identity · Cloud NAT · Artifact Registry · Helm · GitHub Actions

## 📊 GitHub Activity

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Reeceakhun&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" width="48%" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Reeceakhun&theme=tokyonight&hide_border=true" width="48%" />
</p>

<p align="center">
  <img src="./profile/streak.svg" alt="GitHub Streak" width="48%" />
</p>
